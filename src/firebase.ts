import admin from 'firebase-admin';

let rawPrivateKey = process.env.FIREBASE_PRIVATE_KEY || '';
let serviceAccount: any = null;

// Check if the user pasted the JSON content (possibly missing the first '{')
if (rawPrivateKey.includes('"type":') && rawPrivateKey.includes('"project_id":')) {
  if (!rawPrivateKey.startsWith('{')) rawPrivateKey = '{' + rawPrivateKey;
  if (!rawPrivateKey.endsWith('}')) rawPrivateKey = rawPrivateKey + '}';
  
  try {
    const parsed = JSON.parse(rawPrivateKey);
    serviceAccount = {
      projectId: parsed.project_id,
      clientEmail: parsed.client_email,
      privateKey: parsed.private_key,
    };
  } catch (e) {
    console.error("Failed to parse FIREBASE_PRIVATE_KEY as JSON:", e);
  }
} else if (rawPrivateKey) {
  // Fallback to treating it as just the private key string
  serviceAccount = {
    projectId: "boevik-1e8c3",
    clientEmail: "firebase-adminsdk-fbsvc@boevik-1e8c3.iam.gserviceaccount.com",
    privateKey: rawPrivateKey.replace(/\\n/g, '\n'),
  };
} else {
  // Hardcoded fallback
  serviceAccount = {
    projectId: "boevik-1e8c3",
    clientEmail: "firebase-adminsdk-fbsvc@boevik-1e8c3.iam.gserviceaccount.com",
    privateKey: `-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----`
  };
}

if (!admin.apps.length && serviceAccount) {
  try {
    admin.initializeApp({
      credential: admin.credential.cert(serviceAccount),
      databaseURL: `https://${serviceAccount.projectId}-default-rtdb.europe-west1.firebasedatabase.app/`
    });
  } catch (e) {
    console.error("Failed to initialize Firebase Admin:", e);
    // Try fallback URL if europe-west1 fails
    try {
      admin.initializeApp({
        credential: admin.credential.cert(serviceAccount),
        databaseURL: `https://${serviceAccount.projectId}-default-rtdb.firebaseio.com/`
      });
    } catch (e2) {
      console.error("Failed to initialize Firebase Admin with fallback URL:", e2);
    }
  }
}

export const db = admin.apps.length ? admin.database() : {
  ref: () => ({
    once: async () => ({ val: () => null, exists: () => false }),
    set: async () => {},
    update: async () => {},
    remove: async () => {},
    push: () => ({ key: 'mock_key_' + Date.now() }),
    orderByChild: () => ({ equalTo: () => ({ once: async () => ({ val: () => null, exists: () => false }) }) })
  })
} as any;

export const firestore = admin.apps.length ? admin.firestore() : {} as any;
